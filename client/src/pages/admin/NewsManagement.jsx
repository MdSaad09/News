...
import { FiUpload, FiPlus, FiSearch, FiFile, FiTrash2, FiCheck, FiEdit, FiDownload, FiFilter, FiFileText, FiX, FiGrid, FiList } from 'react-icons/fi';
...
const NewsManagement = () => {
  ...
  }, [user, page, statusFilter, categoryFilter]);
    ...
    try {
      ...
      for (const item of validItems) {
        try {
          ...
          const summary = item.summary || item.content.substring(0, Math.min(150, item.content.length)) + '...';
          ...
          if (itemError.response?.status === 400) {
            toast.error(`Validation error for "${item.title}": ${itemError.response.data.message || 'Missing required fields'}`);
          } else if (itemError.response?.status === 413) {
            toast.error(`File too large for "${item.title}". Try a smaller image.`);
          }
        }
      }
  ...
  const handleDeleteNews = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/news/${selectedNews._id}`, config);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredNews.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10 mr-4">
                          <img 
                            ...
                            src={item.coverImage || item.imageUrl || 'https://via.placeholder.com/40x40?text=No+Image'} 
                            alt={item.title}
                            onError={(e) => {
                        ...
                        <div className="text-sm font-medium text-gray-900 max-w-sm truncate">{item.title}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${item.isPublished ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
      ...
      {showDeleteModal && selectedNews && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="mb-6">Are you sure you want to delete the news article "{selectedNews.title}"? This action cannot be undone.</p>
